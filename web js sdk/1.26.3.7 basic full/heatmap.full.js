(function() {
  var $ = function(window) {
    ...
    var urlRegex = /^((https|http|ftp|rtsp|mms)?:\/\/)[^\s]+/;
  ...
  (function() {
    ...
    var heatmap_render = {
      ...
      getCurrentUrl: function() {
        ...
        obj['sa-request-url'] = sessionStorage.getItem('universal_heatmap_url');
        obj['sa-request-url'] = obj['sa-request-url'] ? encodeURIComponent(obj['sa-request-url']) : '';
        obj['sa-request-id'] = sessionStorage.getItem('universal_heatmap_id');
        obj['sa-request-type'] = sessionStorage.getItem('universal_heatmap_type') || '1';
      ...
      setHeatState: function(data, type, url, isFirst) {
        if (isFirst) {
          ...
          var obj = {
            ...
            'sa-request-url': sessionStorage && sessionStorage.getItem ? sessionStorage.getItem('universal_heatmap_url') || '' : ''
          };
          try {
            var windowNameParam = {};
          ...
          if (this.requestType == 1) {
            href.addQueryString(obj);
            if (urlRegex.test(href.getUrl())) {
              location.href = href.getUrl();
            } else {
              console.error("非法重定向尝试");
            }
          } else {
            sessionStorage && sessionStorage.setItem && sessionStorage.setItem('universal_heatmap_type', type);
            location.reload();
          }
        }
      },
      ...
      calculateHeatData: function(data) {
        ...
        if (!_.isObject(data) || !Array.isArray(data.rows) || !_.isObject(data.rows[0])) {
      ...
      setCssStyle: function() {
        ...
        css += '#sa_heat_float_right_box_content table td { color: #fff !important; font-size: 13px !important; background: #333 !important;}';
    ...
    var heatmap = {
          ...
          _.ajax({
            ...
            error: function(res) {
              if (_.isObject(res) && res.error) {
                ...
                sdWarn('AJAX 请求失败，请查看跨域转发配置', res);
              }
            },
            timeout: 10000
          });
            ...
            error: function(err) {
              ...
              if (err === 'timeout') {
                ...
                if (_.URL(location.href).protocol === 'https:' && _.URL(obj.url.jsonp).protocol === 'http:') {
                  error({
                    error: '该页面协议为 https ，请使用 https 的后台查看热力图'
                  });
                } else {
                  error({
                    error: '<div>后台 JSONP 地址不通，可能原因如下：</div><div style="font-size:15px;margin-top:10px;text-align:left;display:inline-block"><div>1. 检查分析版本是否低于 v2.2.0，如果低于此版本请联系值班同学升级版本</div><div style="margin-top:5px">2. 分析后台不允许访问，请贵司研发参考当前页面的控制台报错，去对应的后台配置解决</div></div>'
                  });
                }
              }
            }
        ...
        _.bindReady(function() {
          if (window && window.parent && window.parent.window && window !== window.parent.window) {
            ...
            window.parent.window.postMessage({
                method: 'setUrl',
                params: {
                  request_type: sessionStorage.getItem('universal_heatmap_type') || '1',
                  url: _.getURL()
                }
              },
              sd.para.web_url
      ...
      prepare: function(data, type, url) {
        ...
        if (!web_url && _.sessionStorage.isSupport() && sessionStorage.getItem && sessionStorage.getItem('universal_heatmap_url')) {
          web_url = sessionStorage.getItem('universal_heatmap_url') || null;
        }
