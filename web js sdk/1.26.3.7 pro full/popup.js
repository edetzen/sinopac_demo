(function (global, factory) {
  ...
  popup.config = {
    storageName: 'universalstatistics202002-popupdata',
    loadedSign: 'UniversalStatistics2015JSSDKH5PopupIsLoad'
  };
  ...
  ElementRender.prototype = {
    ...
    render: function () {
      ...
      that.maskEle = that.getElement({
        nodeName: 'div',
        attr: { 'data-universal-click': true },
        style: that.getStyle({
          position: 'fixed',
          width: '100%',
          height: '100%',
      ...
      function actionTypeHandle(properties, params) {
        var actionArrData = properties.maskAction.H5;
        var actionData = _.isArray(actionArrData) && actionArrData[0];
        if (_.isObject(actionData)) {
          params.$sf_close_type = actionData.$sf_close_type || '';
          params.$sf_msg_element_action = actionData.type || '';
          switch (actionData.type) {
            case 'copy':
              popup.track.trackEvent('$PlanPopupClick', params, that.msg);
              break;
            case 'customize':
            case 'openlink':
              popup.track.trackEvent('$PlanPopupClick', params, that.msg);
              if (actionData.closeable) {
                that.destory();
              }
              break;
            case 'close':
      ...
      _.addEvent(that.maskEle, 'click', function (e) {
        ...
        var params = {
          ...
          $sf_msg_element_action: 'close',
          $sf_msg_element_type: 'mask',
          $sf_msg_action_id: properties.maskActionId
        };
        if (mask_ele && properties && properties.maskAction) {
          actionTypeHandle(properties, params);
          return false;
        } else if (mask_ele && that.properties.maskCloseEnabled) {
          popup.track.trackEvent('$PlanPopupClick', params, msg);
          that.destory();
          return false;
        }
      ...
      _.contentLoaded(window, function () {
        that.appendPopup();
      });
    ...
    appendPopup: function () {
      if (window.self === window.top) {
        ...
        try {
          window.top.document.body.appendChild(this.maskEle);
        } catch (e) {
          document.body.appendChild(this.maskEle);
        }
      }

      var boxH = document.querySelector('[data-ele-mask] > div');
      var viewport_height = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
      if (boxH && boxH.offsetHeight > viewport_height) {
        boxH.style.marginTop = 'auto';
      }
    },
    ...
    createView: function (template) {
      ...
      if (template.properties.text) {
        prop.innerText = template.properties.text;
    ...
    destory: function () {
      ...
      popup.info.popup_campaign_listener.onEnd(popup.getSFCampaign(this.msg.plan));
  ...
  popup.ElementRender = ElementRender;
  ...
  popup.handlerCampaign = function (CheckInstance) {
    ...
    if (that.plan.strategy_id) {
      if (that.plan.is_trigger) {
        if (!start) {
          ...
          } else if (popupTemplate.popup_type === 'PRESET') {
            if (!(_.isObject(content) && _.isObject(content.properties) && _.isObject(content.template))) {
              campaign.state = 'DIALOG_NOT_SHOW_JSON_FAILED';
            } else {
              campaign.state = 'DIALOG_SHOW';
            }
          } else {
            campaign.state = 'DIALOG_NOT_SHOW_JSON_FAILED';
          }
      ...
      if (that.plan.is_control_group) {
        campaign.state = 'DIALOG_NOT_SHOW';
      } else if (!start) {
        campaign.state = 'CAMPAIGN_NOT_START_LISTENER_START';
      } else if (!(_.isObject(content) && content.properties && content.template)) {
        campaign.state = 'DIALOG_NOT_SHOW_JSON_FAILED';
      } else {
        campaign.state = 'DIALOG_SHOW';
      }
    ...
    switch (campaign.state) {
      ...
      case 'CAMPAIGN_NOT_START_TRIGGER':
        ...
        break;
      case 'DIALOG_NOT_SHOW':
        that.popupFailed(1003, campaign.isCustom, info);
        break;
      default:
      ...
      _.ajax({
        ...
        success: function (data) {
          ...
          var param = {
            ...
            type: data.popup_type || 'PRESET'
          };
          if (Object.hasOwnProperty.call(data, 'name')) {
            param.name = data.name;
          }

          if (data.popup_type && data.popup_type === 'CUSTOMIZED') {
            ...
            if (_.isObject(content) && _.isObject(content.properties) && _.isObject(content.template)) {
              var ele = new popup.ElementRender(content);
              ele.render();
              popup_info.$sf_succeed = true;
              popup.track.popupDisplay(popup_info);
              popup.info.popup_campaign_listener.onStart(param);
              popup.info.popup_listener.onLoadSuccess(); 
            } else {
              popup_info.$sf_succeed = false;
              popup_info.$sf_fail_reason = '预览信息解析失败，请检查计划配置';
              popup.track.popupDisplay(popup_info);
              popup.info.popup_listener.onLoadFailed('', 1001, '预览信息解析失败，请检查计划配置');
              popup.info.popup_campaign_listener.onFailed(param, 1001, '预览信息解析失败，请检查计划配置');
            }
          }
        },
  ...
  function initPopup() {
    ...
    if (window.screen.width && Number(window.screen.width) > 1024) {
      if (typeof console === 'object' && console.log) {
        console.log('H5 弹窗仅支持移动端和 ipad，或者屏幕宽度小于等于 1024。');
      }
      return false;
    }
  ...
  popup.plugin_name = 'Popup';
  if (window.UniversalStatistics201505 && window.UniversalStatistics201505.modules["Popup"]) {
    window.UniversalStatistics201505.modules.Popup = window.UniversalStatistics201505.modules.Popup || popup;
  } else {
    window.UniversalStatistics201505.modules.Popup = popup;
  }
